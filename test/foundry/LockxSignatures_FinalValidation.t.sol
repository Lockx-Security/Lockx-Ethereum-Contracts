...
contract LockxSignatureFinal is Test {
    ...
    function test_purge_auth_on_burn() public {
        ...
        // First withdraw all ETH (1 ether) before burning
        uint256 currentNonce = _getCurrentNonce(tokenId);
        bytes memory withdrawData = abi.encode(tokenId, 1 ether, user1, bytes32("withdraw"), user1, expiry);
        bytes32 withdrawHash = _computeMessageHash(tokenId, 1, withdrawData, currentNonce);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(key1, withdrawHash);
        bytes memory withdrawSig = abi.encodePacked(r, s, v);
        
        vm.prank(user1);
        lockx.withdrawETH(tokenId, withdrawHash, withdrawSig, 1 ether, user1, bytes32("withdraw"), expiry);
        
        // Now burn the lockbox to trigger _purgeAuth (line 151)
    ...
    function test_all_operation_types() public {
        ...
        // First withdraw remaining ETH (4 ether) before burning
        currentNonce = _getCurrentNonce(tokenId);
        data = abi.encode(tokenId, 4 ether, user1, bytes32("withdraw_remaining"), user1, expiry);
        messageHash = _computeMessageHash(tokenId, 1, data, currentNonce);
        (v, r, s) = vm.sign(key2, messageHash);
        signature = abi.encodePacked(r, s, v);
        
        vm.prank(user1);
        lockx.withdrawETH(tokenId, messageHash, signature, 4 ether, user1, bytes32("withdraw_remaining"), expiry);
