...
contract LockxWithdrawalsUltimate100 is Test {
    ...
    function test_successful_swap_paths() public {
        ...
        lockx.swapInLockbox(
            tokenId, messageHash, signature, address(tokenA), address(tokenB), 1000e18,
            950e18, address(swapRouter), swapData, bytes32("erc20_to_erc20"), expiry, address(0)
        );
        ...
        lockx.swapInLockbox(
            ...
            1900e18, address(swapRouter), swapData, bytes32("eth_to_erc20"), expiry, user2
        );
        
        // Test 3: ERC20 to ETH swap, output to external recipient
        swapData = abi.encodeWithSignature(
            "swap(address,address,uint256,uint256,address)",
            address(tokenA), address(0), 500e18, 0.4 ether, address(lockx)
        );
        
        currentNonce = _getCurrentNonce(tokenId);
        data = abi.encode(
            tokenId, address(tokenA), address(0), 500e18, 0.4 ether,
        ...
        lockx.swapInLockbox(
            ...
            0.4 ether, address(swapRouter), swapData, bytes32("erc20_to_eth"), expiry, user2
        );
    ...
    function test_fee_on_transfer_swap_scenarios() public {
        ...
        bytes memory swapData = abi.encodeWithSignature(
            "swap(address,address,uint256,uint256,address)",
            address(feeToken), address(tokenB), 1000e18, 950e18, address(lockx)
        );
        
        uint256 currentNonce = _getCurrentNonce(tokenId);
        bytes memory data = abi.encode(
            tokenId, address(feeToken), address(tokenB), 1000e18, 950e18,
        ...
        lockx.swapInLockbox(
            ...
            950e18, address(swapRouter), swapData, bytes32("fee_swap"), expiry, address(0)
        );
