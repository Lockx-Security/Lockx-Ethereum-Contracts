...
contract LockxUltimate100 is Test {
    ...
    function test_deposits_ultimate_100() public {
        ...
        uint256 actualReceived = 9000e18; // After 10% fee
    ...
    function test_withdrawals_ultimate_100() public {
        ...
        bytes memory swapData = abi.encodeWithSignature(
            ...
            address(tokenC), address(tokenA), 20000e18, 19000e18, address(lockx)
        );
        
        data = abi.encode(
            tokenId, address(tokenC), address(tokenA), 20000e18, 19000e18,
        ...
        lockx.swapInLockbox(
            ...
            19000e18, address(swapRouter), swapData, bytes32("swap_complete"), expiry, address(0)
        );
    ...
    function test_lockx_ultimate_100() public {
        ...
        bytes memory data = abi.encode(tokenId1, bytes32("burn"), user1, expiry);
        bytes32 messageHash = _computeMessageHash(tokenId1, 4, data, currentNonce);
        ...
        // Test getFullLockbox
        vm.prank(user2);
        (uint256 ethBal, Lockx.erc20Balances[] memory erc20s, Lockx.nftBalances[] memory nftList) = lockx.getFullLockbox(tokenId2);
        assertEq(ethBal, 0);
