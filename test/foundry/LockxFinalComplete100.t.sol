...
contract LockxFinalComplete100 is Test {
    ...
    function test_withdrawals_complete_coverage() public {
        ...
        lockx.swapInLockbox(
            ...
            4750e18, address(swapRouter), swapData, bytes32("token_swap"), expiry, address(0)
        );
        
        // ETH to Token, output to external recipient (950 tokens per ETH)
        swapData = abi.encodeWithSignature(
            "swap(address,address,uint256,uint256,address)",
            address(0), address(tokenB), 5 ether, 4750e18, address(lockx)
        );
        
        currentNonce = _getCurrentNonce(tokenId);
        data = abi.encode(
            tokenId, address(0), address(tokenB), 5 ether, 4750e18,
        ...
        lockx.swapInLockbox(
            ...
            4750e18, address(swapRouter), swapData, bytes32("eth_to_token"), expiry, user3
        );
        
        // Token to ETH (0.01 ETH per 1 token)
        swapData = abi.encodeWithSignature(
            "swap(address,address,uint256,uint256,address)",
            address(tokenB), address(0), 5000e18, 50e18, address(lockx)
        );
        
        currentNonce = _getCurrentNonce(tokenId);
        data = abi.encode(
            tokenId, address(tokenB), address(0), 5000e18, 50e18,
        ...
        lockx.swapInLockbox(
            ...
            50e18, address(swapRouter), swapData, bytes32("token_to_eth"), expiry, user3
        );
