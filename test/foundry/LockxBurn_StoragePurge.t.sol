...
contract LockxBurnPurge is Test {
    ...
    function test_burn_purges_storage() public {
        ...
        // First withdraw ETH (2 ether) before burning
        bytes memory withdrawData = abi.encode(tokenId, 2 ether, user, bytes32("withdraw"), user, expiry);
        bytes32 withdrawStructHash = keccak256(
            abi.encode(OPERATION_TYPEHASH, tokenId, nonce, uint8(1), keccak256(withdrawData))
        );
        bytes32 withdrawDigest = _hashTyped(withdrawStructHash);
        bytes memory withdrawSig = _sign(withdrawDigest);
        
        vm.prank(user);
        lockx.withdrawETH(tokenId, withdrawDigest, withdrawSig, 2 ether, user, bytes32("withdraw"), expiry);
        
        // Update nonce for burn
        vm.prank(user);
        nonce = lockx.getNonce(tokenId);
