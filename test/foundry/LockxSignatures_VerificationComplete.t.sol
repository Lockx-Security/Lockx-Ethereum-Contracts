...
contract LockxSignatureVerificationComplete is Test {
    ...
    function test_burnLockbox_purgeAuth() public {
        ...
        // First withdraw all ETH (3 ether) before burning
        vm.prank(user2);
        uint256 currentNonce = lockx.getNonce(tokenId);
        
        bytes memory withdrawData = abi.encode(tokenId, 3 ether, user2, bytes32("withdraw"), user2, expiry);
        bytes32 withdrawHash = _computeMessageHash(tokenId, 1, withdrawData, currentNonce);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(key2, withdrawHash);
        bytes memory withdrawSig = abi.encodePacked(r, s, v);
        
        vm.prank(user2);
        lockx.withdrawETH(tokenId, withdrawHash, withdrawSig, 3 ether, user2, bytes32("withdraw"), expiry);
        
        // Now create burn signature
        bytes memory data = abi.encode(
