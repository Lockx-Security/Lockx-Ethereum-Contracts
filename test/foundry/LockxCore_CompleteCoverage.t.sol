...
contract LockxCoreCoverageComplete is Test {
    ...
    function test_burnLockbox_success() public {
        ...
        // Withdraw ETH (1 ether)
        bytes memory data = abi.encode(tokenId, 1 ether, user1, bytes32("withdraw_eth"), user1, expiry);
        bytes32 messageHash = _computeMessageHash(tokenId, 1, data, currentNonce);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(key1, messageHash);
        bytes memory signature = abi.encodePacked(r, s, v);
        
        vm.prank(user1);
        lockx.withdrawETH(tokenId, messageHash, signature, 1 ether, user1, bytes32("withdraw_eth"), expiry);
        
        // Withdraw ERC20 (1000e18 tokenA)
        vm.prank(user1);
        currentNonce = lockx.getNonce(tokenId);
        data = abi.encode(tokenId, address(tokenA), 1000e18, user1, bytes32("withdraw_token"), user1, expiry);
        messageHash = _computeMessageHash(tokenId, 2, data, currentNonce);
        (v, r, s) = vm.sign(key1, messageHash);
        signature = abi.encodePacked(r, s, v);
        
        vm.prank(user1);
        lockx.withdrawERC20(tokenId, messageHash, signature, address(tokenA), 1000e18, user1, bytes32("withdraw_token"), expiry);
        
        // Withdraw NFT (ID 2)
        vm.prank(user1);
        currentNonce = lockx.getNonce(tokenId);
        data = abi.encode(tokenId, address(nft), 2, user1, bytes32("withdraw_nft"), user1, expiry);
        messageHash = _computeMessageHash(tokenId, 3, data, currentNonce);
        (v, r, s) = vm.sign(key1, messageHash);
        signature = abi.encodePacked(r, s, v);
        
        vm.prank(user1);
        lockx.withdrawERC721(tokenId, messageHash, signature, address(nft), 2, user1, bytes32("withdraw_nft"), expiry);
    ...
    function test_finalizeBurn_internal() public {
        ...
        // Withdraw all assets before complex burn
        vm.startPrank(user1);
        
        // Withdraw ETH (2 ether)
        bytes memory data = abi.encode(tokenId, 2 ether, user1, bytes32("withdraw_eth"), user1, expiry);
        bytes32 messageHash = _computeMessageHash(tokenId, 1, data, 1);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(key1, messageHash);
        bytes memory signature = abi.encodePacked(r, s, v);
        
        lockx.withdrawETH(tokenId, messageHash, signature, 2 ether, user1, bytes32("withdraw_eth"), expiry);
        
        // Withdraw tokenA (500e18) - nonce 2
        data = abi.encode(tokenId, address(tokenA), 500e18, user1, bytes32("withdraw_tokenA"), user1, expiry);
        messageHash = _computeMessageHash(tokenId, 2, data, 2);
        (v, r, s) = vm.sign(key1, messageHash);
        signature = abi.encodePacked(r, s, v);
        
        lockx.withdrawERC20(tokenId, messageHash, signature, address(tokenA), 500e18, user1, bytes32("withdraw_tokenA"), expiry);
        
        // Withdraw tokenB (300e18) - nonce 3  
        data = abi.encode(tokenId, address(tokenB), 300e18, user1, bytes32("withdraw_tokenB"), user1, expiry);
        messageHash = _computeMessageHash(tokenId, 2, data, 3);
        (v, r, s) = vm.sign(key1, messageHash);
        signature = abi.encodePacked(r, s, v);
        
        lockx.withdrawERC20(tokenId, messageHash, signature, address(tokenB), 300e18, user1, bytes32("withdraw_tokenB"), expiry);
        
        // Withdraw NFT ID 3 - nonce 4
        data = abi.encode(tokenId, address(nft), 3, user1, bytes32("withdraw_nft3"), user1, expiry);
        messageHash = _computeMessageHash(tokenId, 3, data, 4);
        (v, r, s) = vm.sign(key1, messageHash);
        signature = abi.encodePacked(r, s, v);
        
        lockx.withdrawERC721(tokenId, messageHash, signature, address(nft), 3, user1, bytes32("withdraw_nft3"), expiry);
        
        // Withdraw NFT ID 4 - nonce 5
        data = abi.encode(tokenId, address(nft), 4, user1, bytes32("withdraw_nft4"), user1, expiry);
        messageHash = _computeMessageHash(tokenId, 3, data, 5);
        (v, r, s) = vm.sign(key1, messageHash);
        signature = abi.encodePacked(r, s, v);
        
        lockx.withdrawERC721(tokenId, messageHash, signature, address(nft), 4, user1, bytes32("withdraw_nft4"), expiry);
        
        vm.stopPrank();
        
        // Now burn the empty lockbox - nonce 6
        data = abi.encode(tokenId, bytes32("complex_burn"), user1, expiry);
        messageHash = _computeMessageHash(tokenId, 4, data, 6);
        (v, r, s) = vm.sign(key1, messageHash);
        signature = abi.encodePacked(r, s, v);
    ...
    function test_error_conditions() public {
        ...
        // Suppress unused return value warning
        (address(lockx).call(abi.encodeWithSignature("nonExistentFunction()")));
        // Expecting revert via expectRevert()
