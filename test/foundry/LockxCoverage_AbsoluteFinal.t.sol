...
contract LockxAbsoluteFinal100 is Test {
    ...
    function test_signatureVerification_exact_3_lines() public {
        ...
        // Withdraw ETH before burn (5 ether)
        currentNonce = _getCurrentNonce(tokenId);
        data = abi.encode(tokenId, 5 ether, user1, bytes32("withdraw_eth"), user1, expiry);
    ...
    function test_withdrawals_exact_13_lines() public {
        ...
        swapData = abi.encodeWithSignature(
            "swap(address,address,uint256,uint256,address)",
            address(feeToken), address(tokenA), 20000e18, 18960e18, address(lockx)
        );
        
        data = abi.encode(
            tokenId, address(feeToken), address(tokenA), 20000e18, 18960e18,
        ...
        lockx.swapInLockbox(
            ...
            18960e18, address(swapRouter), swapData, bytes32("complete_swap"), expiry, address(0)
        );
