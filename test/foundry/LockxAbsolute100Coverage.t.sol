...
/**
 * @title LockxAbsolute100Coverage
 * @notice ABSOLUTE 100% COVERAGE - Hit every single line in all 4 core contracts
 */
contract LockxAbsolute100Coverage is Test {
    Lockx public lockx;
    ...
    function test_lockx_100_percent() public {
        ...
        // Test getFullLockbox
        vm.prank(user1);
        (uint256 ethBal, Lockx.erc20Balances[] memory erc20s, Lockx.nftBalances[] memory nftList) = lockx.getFullLockbox(tokenId1);
    ...
    function test_withdrawals_100_percent() public {
        ...
        swapData = abi.encodeWithSignature(
            "swap(address,address,uint256,uint256,address)",
            address(tokenA), address(tokenB), 5000e18, 4750e18, address(lockx)
        );
        
        currentNonce = _getCurrentNonce(tokenId);
        data = abi.encode(
            tokenId, address(tokenA), address(tokenB), 5000e18, 4750e18,
        ...
        lockx.swapInLockbox(
            tokenId, messageHash, signature, address(tokenA), address(tokenB), 5000e18,
            4750e18, address(swapRouter), swapData, bytes32("t2t"), expiry, address(0)
        );
        
        // ETH to Token (output to external)
        swapData = abi.encodeWithSignature(
            "swap(address,address,uint256,uint256,address)",
            address(0), address(tokenB), 2 ether, 1900e18, address(lockx)
        );
        
        currentNonce = _getCurrentNonce(tokenId);
        data = abi.encode(
            tokenId, address(0), address(tokenB), 2 ether, 1900e18,
        ...
        lockx.swapInLockbox(
            ...
            1900e18, address(swapRouter), swapData, bytes32("e2t"), expiry, user3
        );
        
        // Token to ETH (output to external)
        swapData = abi.encodeWithSignature(
            "swap(address,address,uint256,uint256,address)",
            address(tokenB), address(0), 1000e18, 10e18, address(lockx)
        );
        
        currentNonce = _getCurrentNonce(tokenId);
        data = abi.encode(
            tokenId, address(tokenB), address(0), 1000e18, 10e18,
        ...
        lockx.swapInLockbox(
            ...
            10e18, address(swapRouter), swapData, bytes32("t2e"), expiry, user3
        );
