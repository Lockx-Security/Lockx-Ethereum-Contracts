...
contract LockxSignatureBinding is Test {
    ...
    function setUp() public {
        ...
        tokenB.mint(address(router), 1000000e18); // 1M tokens instead of 1e26
        tokenA.mint(user, 100000e18); // 100k tokens instead of 1e24
        vm.prank(user);
        tokenA.approve(address(lockx), type(uint256).max);
        vm.prank(user);
        lockx.createLockboxWithERC20(user, key, address(tokenA), 50000e18, ref); // 50k tokens
    }
